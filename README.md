...
<img src="docs/images/logo.png" alt="" width="72" height="72">
...
[![CI](https://github.com/fooxytv/CooldownManagerClassic/actions/workflows/ci.yaml/badge.svg)](https://github.com/fooxytv/CooldownManagerClassic/actions/workflows/ci.yaml)
[![Build and Publish](https://github.com/fooxytv/CooldownManagerClassic/actions/workflows/main.yaml/badge.svg)](https://github.com/fooxytv/CooldownManagerClassic/actions/workflows/main.yaml)
[![Latest tag](https://img.shields.io/github/v/tag/fooxytv/CooldownManagerClassic?label=version)](https://github.com/fooxytv/CooldownManagerClassic/tags)

**Classic Era · Season of Discovery · Burning Crusade Classic · Mists of Pandaria Classic**
...
<!-- SCREENSHOT: docs/images/hero-groups-in-combat.png — the four groups in combat, labelled. This is the hero image. -->
...
<!-- GIF: docs/images/picker-drag.gif — opening /cdmc and dragging two or three abilities into a group. -->
...
<!-- GIF: docs/images/buff-bars.gif — buff bars draining, next to the same group rendered as icons. -->
...
<!-- GIF: docs/images/cooldown-bar.gif — a defensive firing, the bar draining, then dimming through its recharge. -->
...
Three gates run on every push, and all three run locally:

```bash
./ci/scripts/lint.sh            # luacheck; needs luarocks install luacheck
python ci/scripts/check_globals.py   # every client API must be in .luacheckrc
python ci/tests/smoke_test.py        # loads the addon against a stubbed client
```

`develop` is the integration branch; `main` is release-only.

To put a branch on a local client without a toolchain, see [`ci/scripts/Install-CooldownManager.ps1`](ci/scripts/Install-CooldownManager.ps1) — it installs any branch straight from GitHub and stamps the branch name into the version, so the addon list says which build is loaded.

## Bugs and requests

Please raise anything on [GitHub](https://github.com/fooxytv/CooldownManagerClassic/issues). Running `/cdmc status` first and pasting the output into the report saves a round trip — it prints your flavour, interface version, addon build and what the addon has detected.
